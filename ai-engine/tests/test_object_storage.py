"""Real tests for app/core/object_storage.py (Event-First Cloud Storage Phase 1,
sections 9/21) — the boto3 S3 client wrapper. The real network call itself is mocked
at the boto3.client() boundary (mirrors this project's own established convention —
see test_recorder.py's ffmpeg-subprocess mocking); everything above that boundary
(key construction, endpoint/credential selection, bucket naming, the internal-vs-
public client split) is real and unmocked.
...
that fixture's per-test monkeypatch.setattr call ever runs — and calls those
references directly instead of going through `object_storage.<name>`, which would
otherwise resolve to the mock installed for this test too.
"""
...
def _reset_clients():
    """Each test gets a fresh lazily-constructed client so one test's fake boto3
    stub can't leak into the next via the module-level _client/_presign_client cache."""
...
def test_get_client_uses_internal_endpoint_and_configured_credentials(monkeypatch):
    ...
    assert capture["kwargs"]["aws_secret_access_key"] == "testsecret"
    assert capture["kwargs"]["region_name"] == "us-west-2"
    get_settings.cache_clear()


def test_get_client_is_memoized_across_calls(monkeypatch):
    capture: dict = {}
    _patch_boto_client(monkeypatch, capture)
...
def test_generate_presigned_url_uses_public_endpoint_not_internal(monkeypatch):
    from app.config import get_settings
    ...
    # ai-engine's Settings doesn't declare s3_public_endpoint_url at all (only the
    # backend needs it — see object_storage.py's own docstring), so getattr(...) falls
    # back to the internal endpoint here. This test documents that real, current
    # behavior rather than assuming the field exists.
    assert capture["kwargs"]["endpoint_url"] == "http://minio:9000"
...
def test_ensure_bucket_exists_is_a_noop_when_bucket_already_exists(monkeypatch):
    class _AlreadyExistsClient(_FakeBotoClient):
        def head_bucket(self, Bucket):
            ...
            return {}  # bucket exists — no exception
    ...
    object_storage.ensure_bucket_exists()  # must not raise
